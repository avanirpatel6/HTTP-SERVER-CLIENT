...
void serv(int n)
{
	char message[99999], *requested[3], dts[BYTES], path[99999];
	int rcvd, fd, read_bytes;
	memset((void *)message, (int)'\0', 99999);
	rcvd = recv(clients[n], message, 99999, 0);
	if (rcvd < 0)
	{
	...
	else
	{
		...
		{
			requested[0] = strtok(message, " \t\n");

			if (strncmp(requested[0], "GET\0", 4) == 0)
			{
				requested[1] = strtok(NULL, " \t");
				requested[2] = strtok(NULL, " \t\n");

				if (strncmp(requested[2], "HTTP/1.0", 8) != 0 && strncmp(requested[2], "HTTP/1.1", 8) != 0)
				{
					write(clients[n], "HTTP/1.0 400 Bad Request\n", 25);
				}
				else
				{
					if (strncmp(requested[1], "/\0", 2) == 0)
					{
						requested[1] = "/index.html";
					}

					strcpy(path, ROOT);
					strcpy(&path[strlen(ROOT)], requested[1]);
